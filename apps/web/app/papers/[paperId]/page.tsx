...
import { NumberBadge, ProvenanceBadge, SourceChip, Tag } from "../../components/ui";
...
function AskPanel() {
  ...
  return <aside className="ask-panel" id="ask-paper">
    ...
    <div className="citation-row"><SourceChip href={paperLinks.pdf}>Sec. 3.2 (p. 4)</SourceChip><SourceChip href={paperLinks.pdf}>Sec. 3.1 (p. 3)</SourceChip><SourceChip icon="chart" href={paperLinks.pdf}>Table 1 (p. 7)</SourceChip></div>
    <div className="helpful-row"><span>Was this helpful?</span><button type="button" className={feedback === "helpful" ? "is-active" : ""} aria-label="Helpful" aria-pressed={feedback === "helpful"} onClick={() => setFeedback("helpful")}><Icon name="check" size={16} /></button><button type="button" className={feedback === "not-helpful" ? "is-active" : ""} aria-label="Not helpful" aria-pressed={feedback === "not-helpful"} onClick={() => setFeedback("not-helpful")}><Icon name="close" size={16} /></button>{feedback ? <small className="feedback-status">Thanks for the feedback.</small> : null}</div>
...
function SnapshotCard() {
  return <section className="snapshot-card paper-card"><h2><Icon name="building" size={20} /> Paper Snapshot</h2><div className="snapshot-grid"><span><Icon name="authors" size={16} /><b>Authors</b>Vaswani et al.</span><span><Icon name="calendar" size={16} /><b>Year</b>2017</span><span><Icon name="building" size={16} /><b>Venue</b>NeurIPS</span><span><Icon name="link" size={16} /><b>DOI</b><a href={paperLinks.doi} target="_blank" rel="noreferrer">10.48550/arXiv.1706.03762 <Icon name="external" size={11} /></a></span><span><Icon name="link" size={16} /><b>arXiv</b><a href={paperLinks.abstract} target="_blank" rel="noreferrer">1706.03762 <Icon name="external" size={11} /></a></span><span><Icon name="lock" size={16} /><b>Open Access</b><Tag tone="green">Open Access</Tag></span><span className="snapshot-topics"><Icon name="tag" size={16} /><b>Topics</b><Tag>Deep Learning</Tag><Tag>NLP</Tag><Tag>Sequence Modeling</Tag><Tag>Attention</Tag></span></div><a className="button button-outline download-paper" href={paperLinks.pdf} target="_blank" rel="noreferrer"><Icon name="download" size={16} /> Download PDF <Icon name="external" size={14} /></a></section>;
}
...
function OverviewBlock({ title, icon, children, tone = "" }: { title: string; icon: "sparkles" | "question" | "balance" | "building" | "graduation"; children: React.ReactNode; tone?: string }) {
  ...
  return <section id={id} className={`overview-block paper-card ${tone}`}><h2><a className="overview-block-link" href={`#${id}`}><Icon name={icon} size={20} /> {title}<Icon className="block-chevron" name="chevronRight" size={18} /></a></h2><div>{children}</div></section>;
}
